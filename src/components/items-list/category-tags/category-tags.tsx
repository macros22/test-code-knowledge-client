...
export const CategoryTags = ({itemsInfo, currentCategory, categoryButtonHandler}) => {
    ...
    return (
        ...
        className="keen-slider max-w-[500px] mx-auto"
        >
        {Object.keys(itemsInfo).map((category, idx) => {
          return (
            ...
            //   variant={category == currentCategory ? 'default' : 'outline'}
              variant={'outline'}
              className={`keen-slider__slide border min-w-[120px] max-w-[300px] !w-auto mr-4 hover:text-md cursor-pointer px-3 py-2 ${category == currentCategory ? 'border-primary' : ''}`}
              // className={
